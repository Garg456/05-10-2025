...
public class SC04_Refresh_Menu2 extends BaseTest2 {
...
//		srac.searchProduct(menuname);
	...
	public static void clickONMenuReport(String priceOption, String ReportStartDate, String ReportEndDate) throws Exception {
	    ...
	    // Wait for and click the Download Excel button inside menuRptbtn div
	    wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='menuRptbtn']//button[@title='Download Excel']"))).click();
	    System.out.println("Clicked on Report button");

	    // Wait until the price option buttons appear (to avoid Thread.sleep)
	    if (priceOption.equalsIgnoreCase("With Price")) {
	        WebElement withPriceBtn = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[contains(text(),'With Price')]")));
	        withPriceBtn.click();
	        System.out.println("Clicked 'With Price' button");
	    } else if (priceOption.equalsIgnoreCase("Without Price")) {
	        WebElement withoutPriceBtn = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[contains(text(),'Without Price')]")));
	    ...
	    // Now wait for the download prompt or any element that signals the download started
	    // For example, wait for the "Keep" button or something similar - since this depends on your setup,
	    // here I will use a simple fixed wait as a placeholder:
	    Thread.sleep(5000); // If you can find a better element to wait for, replace this!
	    ...
	    // Wait for download to complete - again ideally wait for a file existence check or UI signal
	    Thread.sleep(10000);
	}
