...
public class TC_CheckRefreshMenuONList extends BaseTest2 {
	...
	public void Test_RefreshMenuONList(String username, String password, String recipeName, String menuname,
		...
		RefreshMenuONList(menuname, sitename, recipeName, Menu_Type, MenuCategory, EffectiveStartDate, EffectiveEndDate,sitename2,priceOption,NoOfCustomer,ReportStartDate,ReportEndDate);
	}

	public static void RefreshMenuONList(String menuname, String sitename, String Recipename, String Menu_Type,
		...
		clicksearchedLink(Recipename);
         editRecipeonsite();
         publishRecipeonsite();
		System.out.println("recipe edit complted");
		...
		System.out.println("recipe site added");
		...
		searchProductOnSearchbar(menuname);
		
		
		validationRefreshMenuOnList(menuname);
	...
	public static void validationRefreshMenuOnList(String menuName) {
	    try {
	        // Trim to avoid accidental spaces
	        String xpath = "//a[contains(text(),'" + menuName.trim() + "')]/preceding-sibling::a";
	        WebElement refreshIcon = driver.findElement(By.xpath(xpath));

	        // Assert that the refresh icon is displayed
	        Assert.assertTrue(refreshIcon.isDisplayed(), "Refresh icon should be visible for menu: " + menuName);
	        System.out.println("✅ Refresh icon is displayed for menu: " + menuName);
	    } catch (NoSuchElementException e) {
	        // Element not found means refresh icon is NOT displayed, so assert false
	        Assert.fail("Refresh icon NOT found for menu: " + menuName);
	    }
	}
	...
	public static void addRecipeOnMenu(String recipeName) {
	    ...
	    // OPTIONAL: Take screenshot for confirmation
	    /*
	    try {
	        File screenshot = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
	        Files.copy(screenshot.toPath(), Paths.get("E:\\TestScreenshots\\after_plus_click.png"));
	        System.out.println("📸 Screenshot captured after plus icon click.");
	    } catch (IOException ioe) {
	        System.out.println("⚠️ Could not take screenshot: " + ioe.getMessage());
	    }
	    */
	...
	public static void validatePlusIcons() {
	    driver.findElement(By.id("tabFiveTab")).click();
	    ...
	    long visibleCount = plusIcons.stream().filter(WebElement::isDisplayed).count();
	    System.out.println("Visible Plus Icons: " + visibleCount);

	    WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
	    long clickableCount = plusIcons.stream()
	        .filter(icon -> {
	            try {
	    ...
	    System.out.println("Clickable Plus Icons: " + clickableCount);
	}
