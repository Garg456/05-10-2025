...
public class TC_CreateSite extends BaseTest2 {
...
public Object[][] recipeDataProvider() throws IOException {
	...
	for (int i = 0; i < dataList.size(); i++) {
		...
		//sitetype,RealSite, sector,siteName, adhocsitelist, view
...
public static void createSite(String sitename,String sitetype,String RealSite,String sector,String siteName,String adhocsitelist,String view ) throws Exception {
	...
	clickSiteManagement();
	clickAddSitebutton();
	clickSitetype(sitetype);
	if(sitetype.equalsIgnoreCase("Real")) {
	
	clickRealSite(RealSite);
...
public static  void clicksiteName(String siteName) {
	...
	driver.findElement(By.xpath("//label[contains(text(),'Site Name')]/following::input[1]")).sendKeys(siteName,Keys.ENTER);
}
